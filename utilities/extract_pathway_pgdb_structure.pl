...
my @subpathways = $metacyc->get_slot_values('|Pathways|', 'Sub-Pathways');

print Dumper(@subpathways);


  # &printTree('|Biosynthesis|',   $metacyc, 0);
...
sub printTree{
   ...
   my @subpathways = $cyc->get_slot_values($node, 'Sub-Pathways');
   print "$node shall\n";
   print Dumper(@subpathways);

   foreach my $subpathway (@subpathways) {
       print "hello\n";
      &printTree($subpathway,  $cyc, $indent+1);
   }
 #  &printTree($cnode, $node,  $cyc, $indent+1);
