...
class Sequence(object):
    ...
    def __repr__(self):
        truncated = (
            self.sequence.replace(self.sequence[10:-10], "...")
...
class NucleotideSequence(Sequence):
    ...
    def __get_reverse_strand(self):
        return self.__class__(
            "".join(
                self.pairings[base] if base in self.pairings else "?"
    ...
    def __get_translations(self):
        for feature in self.features:
            ...
            if (
                ...
                and qualifiers["locus_tag"]
                and qualifiers["translation"]
            ):
                ...
                record = SequenceRecord(
                    locus="".join(qualifiers["locus_tag"]),
                    accession=self.accession,
                    division=self.division,
                    molecule_type=self.molecule_type,
                    date=self.date,
                    definition=(
                        "".join(qualifiers["product"])
...
class Attribute(object):
    ...
    def __repr__(self):
        return "<Attribute %s=%s subattributes=%s>" % (
            self.name,
            repr(self.value),
...
class Feature(object):
    ...
    def __repr__(self):
        return "<Feature %s at %s qualifiers=%s>" % (
            self.type,
            self.location,
