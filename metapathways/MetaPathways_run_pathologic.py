...
try:
    ...
    #from metapathways.pathwaytoolsutils import *
...
def main(argv, errorlogger=None, runcommand=None, runstatslogger=None):
    ...
    if options.inputfolder == None:
        ...
        if gutils.files_exist(files, errorlogger=errorlogger):
    ...
    if not path.exists(options.ptoolsExec):
        gutils.eprintf("ERROR\tPathwayTools executable %s not found!\n", options.ptoolsExec)
        if errorlogger:
            errorlogger.printf(
    ...
    if not path.exists(options.reactions_list):
        try:
            ...
            for line in resultLines:
                gutils.fprintf(reaction_list_file, "%s\n", line.strip())
...
def ExtractPathway_WTD(options):
    ...
    gutils.printf("\n")
    gutils.printf("INFO\tEntering the WTD calculations!\n")
    ...
    try:
        ...
        if options.wtd and not path.isfile(serialized_metacyc_taxa_ranges):
            ...
            # try:
            gutils.printf("INFO\tGetting MetaCyc Expected Taxonomic Range(s)\n")
        ...
        gutils.printf("INFO\tGetting ORF to Taxa Map from AnnotationTable\n")
        orf_lca = {}
    ...
    pwy_to_wtd = {}
    gutils.printf("INFO\tCalculating WTD\n")
...
def MetaPathways_run_pathologic(
    ...
    try:
        ...
        errormod.insert_error(error_code)
