...
class SampleData:
    ...
    def _createLogFiles(self):
        self.runlogger = mputils.WorkflowLogger(
            mputils.generate_log_fp(self.output_dir, basefile_name="metapathways_run_log"),
            open_mode="w",
        )
        self.stepslogger = mputils.WorkflowLogger(
            mputils.generate_log_fp(self.output_dir, basefile_name="metapathways_steps_log"),
            open_mode="a",
        )
        self.errorlogger = mputils.WorkflowLogger(
            mputils.generate_log_fp(self.output_dir, basefile_name="errors_warnings_log"),
            open_mode="a",
        )
        self.runstatslogger = mputils.WorkflowLogger(
            mputils.generate_log_fp(
                self.output_run_statistics_dir,
    ...
    def _createFolders(self):
        ...
        gutils.checkOrCreateFolder(self.output_results_rRNA_dir)
        gutils.checkOrCreateFolder(self.output_results_tRNA_dir)
    ...
    def hasPToolsInput(self):
        ...
        for file in files:
            if not gutils.doesFileExist(self.output_fasta_pf_dir + PATHDELIM + file):
                return False
    ...
    def hasGenbankFile(self):
        ...
        return gutils.doesFileExist(output_annot_gbk)
    ...
    def hasSequinFile(self):
        ...
        return gutils.doesFileExist(output_annot_sequin)
