...
def create_annotation(
    ...
    for contig in gffreader:
        ...
        for orf in gffreader.orf_dictionary[contig]:
            ...
            seqname = orf["seqname"]
            if compact_output:
                orf_id = mputils.ShortenORFId(orf_id)
                seqname = mputils.ShortenContigId(seqname)

            gutils.fprintf(output_table_file, "%s", orf_id)
            gutils.fprintf(output_table_file, "\t%s", orf["orf_length"])
            gutils.fprintf(output_table_file, "\t%s", orf["start"])
            gutils.fprintf(output_table_file, "\t%s", orf["end"])
    ...
    def __init__(self, dbname, blastoutput):
        ...
        try:
            ...
            if not self.seq_beg_pattern.search(line):
                gutils.eprintf('First line must have field header names and begin with "#"\n')
...
def process_parsed_blastoutput(
    dbname, blastparser, cutoffs, annotation_results, pickorfs, callnum=0
):
    fields = [
        "target",
        "q_length",
...
def writeParsedLines(fieldmapHeaderline, parsedLines, list, names, outputfilename):
    ...
    gutils.fprintf(outputfile, "%s", outputStr)
...
def merge_sorted_parsed_files(
    ...
    try:
        ...
        gutils.eprintf("\n\tERROR:\tCannot read sequence file : %s\n", filenames[i])
        mputils.exit_process()
    ...
    try:
        ...
        gutils.eprintf("\n\tERROR:\tCannot create sequence file : %s\n", outputfilename)
        mputils.exit_process()
...
def create_sorted_parse_blast_files(
    ...
    if verbose:
        gutils.eprintf("\n\n\n")
        gutils.eprintf("dbname                   : %s\n", dbname)
        gutils.eprintf("Parsed file              : %s\n", blastoutput)
    ...
    if verbose:
        gutils.eprintf("Number of lines          : %s\n", str(currSize))
        gutils.eprintf("Merge the files          : %s\n", sorted_parse_file)
        gutils.eprintf("Number of files to merge : %s\n", str(len(filenames)))
...
def main(argv, errorlogger=None, runstatslogger=None):
    ...
    lca = lcacompute.LCAComputation(opts.ncbi_taxonomy_map, opts.ncbi_megan_map)
    ...
    with open( opts.output_dir + PATHDELIM + opts.sample_name + ".ORF_annotation_table.txt", "w") as outputfile:
        ...
        while start < Length:
            ...
            for dbname in results_dictionary.keys():
                gutils.eprintf(
                    "\n\tINFO:\tNumber of collected hits in {}: {}".format(
                     dbname, len(results_dictionary[dbname].keys())
                 ))
...
def process_subsys2peg_file(subsystems2peg, subsystems2peg_file):
    ...
    try:
        ...
        errormod.insert_error(errorcode)
        gutils.eprintf("\n\tEOOR:\tCannot close " + str(subsystems2peg_file))
...
def print_orf_table(results, orfToContig, output_dir, outputfile, compact_output=False):
    ...
    for orfn in orf_dict:
        ...
        if compact_output:
            orfName = orfn
            contigName = mputils.ShortenContigId(contigName)
