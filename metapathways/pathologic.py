...
DATABASE_MAPPINGS = {"InterPro": "UNIPROT", "GI": "GI", "GeneID": "GeneID", "GO": "GO"}
...
def feature_to_string(feature):
    ...
    # Initialization
    details = {"product_type": None, "references": [], "go_terms": [], "ec_numbers": []}

    # Determine the product type

    details["product_type"] = (
        PRODUCT_TYPE_MAPPINGS[feature.type]
        if feature.type in PRODUCT_TYPE_MAPPINGS
        else "P"
    )
    ...
    details["coordinates"] = [
        (str(c.begin), str(c.end)) for c in feature.location.coordinates
    ]
        ...
        for xref in feature.db_xrefs:
            ...
            if database in DATABASE_MAPPINGS:
                details["references"].append(
                    "%s:%s" % (DATABASE_MAPPINGS[database], reference)
                )

    # GO terms

    details["go_terms"] = [
        go.split(":")[1] for go in feature.db_xrefs if go.startswith("GO:")
    ]
...
class PathoLogicRecordSerializer(SequenceRecordSerializer):
    ...
    def serialize(self, record):
        filtered_features = (
            ...
            if feature.type in ("CDS", "tRNA", "rRNA", "mRNA", "misc_RNA")
        )
