...
def fprintf(_file, fmt, *args):
    _file.write(fmt % args)
...
def create_splits(
    ...
    fastareader = fastreadrmod.FastaReader(input_filename)
...
def countNoOfSequencesInFile(file):
    fastareader = fastreadrmod.FastaReader(file)
    count = 0
