        ...
        try:
            if compact_output:
                ...
                attributes = (
                    ...
                    + sample_name
                    + "_"
                    + ShortenORFId(orf_dictionary[contig][candidate_orf_pos]["id"])
                )

            attributes += (
                ";"
                + "locus_tag="
                + ShortenORFId(orf_dictionary[contig][candidate_orf_pos]["locus_tag"])
            )
        except:
...
def write_16S_tRNA_gene_info(rRNA_dictionary, outputgff_file, tag):
    ...
    for rRNA in rRNA_dictionary:
        ...
        attributes += (
            ";" + "locus_tag=" + ShortenORFId(rRNA_dictionary[rRNA]["seqname"]) + tag
        )
