...
try:
    ...
    # from metapathways.utils.metapaths_utils  import parse_command_line_parameters, fprintf, printf
    from metapathways import pathwaytoolsutils as ptoolsmod
...
def createParser():
    ...
    wtd_options_group.add_option(
        "--ncbi-megan-map",
        dest="ncbi_megan_map",
        help="MEGANs prefered mapping NCBI IDs",
    )
...
def start_pathway_tools_api_mode(pathway_tools_exe):
    ...
    result = sysutils.getstatusoutput(command)
...
def main(argv):
    ...
    lca = lcacompute.LCAComputation([opts.ncbi_tree])
