...
{        
  ...
  *countBranches = *countBranches + 1;
  ...
  if(originalTree)
    /*sprintf(treestr, ":%8.20f[%s", p->bInf->originalBranchLength, p->bInf->branchLabel);  original line */
    sprintf(treestr, ":1.0[%s",  p->bInf->branchLabel);
  else
    sprintf(treestr, ":1.0[%s", p->bInf->branchLabel);
  while (*treestr) treestr++;
...
void classifyML(tree *tr, analdef *adef)
{
  ...
  printf("Tree stirng print after 1975\n");
  printf("%s\n", tr->tree_string);    
  fprintf(treeFile, "%s\n", tr->tree_string);    
  fclose(treeFile);
