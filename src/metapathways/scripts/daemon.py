...
parser.add_option(
    "--home-dir", dest="home_dir", default="''", help="home dir [REQUIRED]"
)
parser.add_option(
    "--does-sample-dir-exist",
    dest="does_sample_dir_exist",
    default="",
    help="does sample dir exist",
)
parser.add_option(
    "--does-file-exist", dest="does_file_exist", default="", help="does file dir exist"
)

parser.add_option(
    "--does-file-pattern-exist",
    dest="does_file_pattern_exist",
    default="",
...
parser.add_option(
    ...
    dest="get_files_with_pattern",
    default="",
    help="get file names with the pattern supplied",
)

parser.add_option(
    "--get-result-filenames-with-suffix_in_dir",
    dest="get_result_filenames_with_suffix_in_dir",
    default="",
...
parser.add_option(
    ...
    dest="get_result_filenames",
    default="",
    help="get the result file names",
)


parser.add_option("--eof-tag", dest="eof_tag", default="", help="the end of file tag")

parser.add_option(
    "--suffix", dest="suffix", default="", help="the suffix at the end of the filename"
)

parser.add_option("--sample-name", dest="sample_name", default="", help="sample name")

parser.add_option(
    "--create-sample-dir",
    dest="create_sample_dir",
    default="",
...
parser.add_option(
    "--remove-sample-dirs",
    dest="remove_sample_dirs",
    default=[],
    action="append",
    help="remove sample directories",
)

parser.add_option("--remove-file", dest="remove_file", default="", help="remove file")

parser.add_option(
    "--number-of-sequences-in-file",
    dest="number_of_sequences_in_file",
    default="",
...
parser.add_option(
    "--split-into-batches",
    dest="split_into_batches",
    default="",
    help="count the number of sequences in file",
)

parser.add_option(
    "--format-database", dest="format_database", default="", help="formats the database"
)

parser.add_option("--batch-size", dest="batch_size", default=500, help="batch size")
...
parser.add_option("--cpu-type", dest="cpu_type", default="", help="return CPU type")
...
parser.add_option("--submit-job", dest="submit_job", default="", help="submit job")
...
parser.add_option("--memory", dest="memory", default="10gb", help="memory size request")

parser.add_option(
    "--walltime", dest="walltime", default="10:00:00", help="wall time request"
)

parser.add_option(
    "--database-file", dest="database_file", default="", help="database file name"
)
...
parser.add_option("--dbname", dest="dbname", default="", help="dbname")

parser.add_option(
    "--dbnames", dest="dbnames", default=[], action="append", help="dbnames, an array"
)

parser.add_option(
    "--is-complete", dest="is_complete", default="", help="is consolidate"
)
...
parser.add_option(
    ...
    dest="get_number_of_running_jobs",
    default="",
...
def get_number_of_completed(sample_name, algorithm):
    ...
    samples_dictionary = {}
    read_list(
        namePrefix + "completed.txt",
        samples_dictionary,
        filtercol=2,
        filterword=algorithm,
    )
...
def read_filtered_list(
    listfilename, dictionary, col=1, filter_cols=[], filter_by_words=[]
):
    try:
...
def consolidate(sample_name, dbname, algorithm):
    ...
    consolidatedfile = (
        "MetaPathways/" + sample_name + "/" + sample_name + "." + db_output_suffix
    )
    ...
    for smallfile in by_dbnames[dbname]:
        ...
        try:
            append_file_content_from_to(
                smallfile + "." + db_output_suffix, consolidatedfile
            )
        except:
            pass
            # traceback.print_exc(1)
...
def is_complete(sample_name, dbname, algorithm):
    ...
    read_filtered_list(
        ...
        col=1,
        filter_cols=[1, 4],
        filter_by_words=[dbname, algorithm],
    )
    ...
    read_filtered_list(
        ...
        col=1,
        filter_cols=[2, 3],
        filter_by_words=[algorithm, dbname],
    )
    ...
    try:
        ...
        if algorithm == "BLAST":
            command = (
                working_dir
                + "/"
                + "MetaPathways/executables/blastp -num_threads 1  -max_target_seqs 5  -outfmt 6"
                + " -db "
                + ("MetaPathways/databases/" + dbname)
                + " -query "
                + sampleDir
                + "/"
                + split_file
        ...
        if algorithm == "LAST":
            command = (
                working_dir
                + "/"
                + "MetaPathways/executables/lastal"
                + " -o "
                + outputfile
                + " -f 0 "
                + ("MetaPathways/databases/" + dbname)
        ...
        args = [
            ...
            "-e",
            sampleDir + "/.qstatdir/" + shortoutputfilename + random_id + "error.log",
            "-o",
            sampleDir + "/.qstatdir/" + shortoutputfilename + random_id + "output.log",
            commandfileName,
        ]
...
def main(argv):
    ...
    if len(opts.get_result_filenames) > 0:
        try:
            getResultFileNames(
                opts.get_result_filenames, opts.filename_suffix, opts.eof_tag
            )
        except:
            print " "
    ...
    if len(opts.get_result_filenames_with_suffix_in_dir) > 0:
        try:
            getFileNamesWithSuffix(
                opts.get_result_filenames_with_suffix_in_dir,
                opts.filename_suffix,
                opts.eof_tag,
            )
        except:
            print " "
    ...
    if len(opts.split_into_batches) > 0:
        try:
            batch_count = split_into_batches(
                opts.split_into_batches,
                opts.database_files,
                opts.dbnames,
                int(opts.batch_size),
                opts.algorithm,
    ...
    if (
        len(opts.submit_job) > 0
        and len(opts.dbname)
        and len(opts.sample_name)
        and len(opts.algorithm)
    ):
        try:
            submit_job(
                opts.sample_name,
    ...
    if len(opts.format_database) > 0:
        try:
            format_database(
                opts.format_database, opts.algorithm, working_dir=opts.home_dir
            )
        except:
            return 0
    ...
    if len(opts.number_of_lines_in_file) > 0:
        try:
            number_of_lines_in_file(
                opts.number_of_lines_in_file, working_dir=opts.home_dir
            )
        except:
            return 0
