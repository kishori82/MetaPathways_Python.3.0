...
def get_sample_name(gff_file_name):
    ...
    sample_name = re.sub(
        ".annot.gff", "", gff_file_name
    )  # Niels: somewhere we changed the file name?
...
def process_gff_file(
    ...
    if "ptinput" in output_filenames:
        write_ptinput_files(
            output_filenames["ptinput"],
            contig_dict,
            sample_name,
            nucleotide_seq_dict,
            protein_seq_dict,
            compact_output,
            orf_to_taxonid=orf_to_taxonid,
        )


# this function creates the pathway tools input files
def write_ptinput_files(
    output_dir_name,
    contig_dict,
    sample_name,
    nucleotide_seq_dict,
    ...
    for key in contig_dict:
        ...
        for attrib in contig_dict[key]:
            ...
            write_to_pf_file(
                output_dir_name, shortid, attrib, compact_output=compact_output
            )

            # append to the gen elements file
            if compact_output == False:
                append_genetic_elements_file(
                    genetic_elements_file, output_dir_name, shortid
                )
        # endfor
    ...
    rename(
        output_dir_name + "/.tmp.genetic-elements.dat",
...
def write_gbk_file(
    output_file_name, contig_dict, sample_name, nucleotide_seq_dict, protein_seq_dict
):
    date = genbankDate()
...
usage = (
    sys.argv[0]
    + """ -g gff_files -n nucleotide_sequences -p protein_sequences [--out-gbk gbkfile --out-ptinput ptinputdir]\n"""
)
...
def createParser():
    ...
    input_group.add_option(
        "-p",
        "--protein",
        dest="protein_sequences",
        metavar="PROTEIN_SEQUENCE",
        help="Protein sequences",
    )

    input_group.add_option(
        "-o", "--output", dest="output", metavar="OUTPUT", help="Genbank file"
    )
...
def main(argv, errorlogger=None, runstatslogger=None):
    ...
    if options.ncbi_taxonomy_tree != None and options.taxonomy_table != None:
        orf_to_taxonid = read_taxons_for_orfs(
            options.ncbi_taxonomy_tree, options.taxonomy_table
        )
...
def MetaPathways_create_genbank_ptinput_sequin(
    argv, errorlogger=None, runstatslogger=None
):
    createParser()
    global errorcode
    try:
